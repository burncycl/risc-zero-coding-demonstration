...
Internet search `Docker Install` and *insert-your-platform-here* to get more clear references.

Once Docker is installed, to build Dockerized local development environment use,
```
docker build -t aws-deploy-ipcted:latest .
```
This will produce an image on your local system called `aws-deploy-ipcted:latest` to be used henceforth for local development of this project.
...
What's cool about this automation, is it allows you to store your secrets (e.g. AWS Cloud credentials + ssh keys) in source control using Ansible Vault. This also version controls your local development environment. No Dockerized overhead.
...
```
Additionally, check out [This README.md](./ansible-local-development-environment/README.md)
...
- Phase 2 - Cloudfront site using hosted zone on Route 53 with allow list. 
...
KISS solution using a basic S3 bucket with policy permissions.
...
Cloudfront site using hosted zone on Route 53 with allow list.
